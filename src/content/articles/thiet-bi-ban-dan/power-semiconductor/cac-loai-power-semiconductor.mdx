...
<p class="hook">Để chỉnh lưu, bạn dùng diode. Để điều khiển, bạn dùng transistor. Nhưng khi công suất lớn đến mức triệu Watt, bạn cần đến thyristor — thiết bị kỳ lạ nhất: nó có thể bị bật lên nhưng không thể bị tắt đi. Tại sao lại có ba loại hoàn toàn khác nhau như vậy?</p>

## Ba Loại Công Cụ Điều Khiển Dòng Điện

Hãy tưởng tượng bạn cần kiểm soát một con sông nước (dòng điện) chảy qua thung lũng (mạch):

- **Diode** giống một cái cổng cứng: nó chỉ mở một chiều duy nhất, sau khi xây dựng xong thì không thay đổi được.
- **Transistor** giống một van nước tinh vi: bạn có thể điều chỉnh độ mở linh hoạt từng giây, từng nanogiây.
- **Thyristor** giống một chiếc khoá huyền bí: khi bạn cho nó tín hiệu kích hoạt, nó bật vọt lên, nhưng sau đó nó "bị mắc kẹt" ở trạng thái BẬT — chỉ tắt được khi bạn cắt dòng điện hoàn toàn.

## Định Nghĩa Chung

Bán dẫn công suất là các thiết bị "được thiết kế để điều khiển dòng điện lớn và điện áp cao" trong các ứng dụng chuyển đổi và quản lý năng lượng điện. Ba loại chính, mỗi loại có cấu trúc và chức năng riêng biệt.
...
**Chỉnh lưu** (rectification): Cho phép dòng điện chảy chỉ một chiều duy nhất — giống cách một cái cổng chỉ cho người vào, không cho người ra.
...
**Thiên áp thuận**: Vùng cạn kiệt hẹp → dòng điện chảy (điện áp xuôi ~0.7V)

**Thiên áp nghịch**: Vùng cạn kiệt rộng → dòng điện gần như bằng 0 (đóng vai trò chặn dòng)
...
### 2. Power Transistor (Transistor Công Suất)

#### Chức Năng

**Chuyển mạch** (switching): Đóng/mở điện ở tốc độ cao

**Điều khiển dòng điện** lớn nhân một tín hiệu điều khiển nhỏ — bạn có thể điều chỉnh bao nhiêu dòng chảy qua, với độ chính xác cao.
...
Thay vì thay đổi điện áp trực tiếp, transistor bật/tắt nhanh (kHz~MHz). Tỷ lệ thời gian bật/tắt xác định độ lớn điện áp trung bình.
...
### 3. Power Thyristor (Thyristor Công Suất)
...
Không có dòng Gate → Thyristor không dẫn điện

**Kích Hoạt (Triggering)**:

Cấp dòng nhỏ vào Gate (mA~A) → Cơ chế latch-up kích hoạt: BJT phía dưới ON → phát thải carriers → BJT phía trên ON → phản hồi dương. Khi latch-up, dòng Anode-Cathode tăng vọt (kA~MA)

**Trạng thái BẬT (ON)**:

**Đặc biệt**: Ngay cả khi bỏ tín hiệu Gate, dòng tiếp tục chảy. Thyristor "giữ" trạng thái ON miễn là dòng Anode > dòng giữ tối thiểu (holding current). Chỉ tắt được bằng cách giảm dòng Anode xuống dưới dòng giữ.
...
<details>
<summary>Chi tiết về ưu/hạn chế của Thyristor</summary>
...
<QuickCheck
	client:visible
	question="Điểm khác biệt cơ bản giữa Transistor và Thyristor là gì?"
	options={[
		{ label: "Transistor có thể bị tắt bằng tín hiệu Gate; Thyristor không thể, phải cắt dòng Anode", correct: true },
		{ label: "Transistor chỉ chịu áp thấp, Thyristor chịu áp cao", correct: false },
		{ label: "Transistor là diode, Thyristor là transistor", correct: false },
	]}
	explanation="Sự khác biệt then chốt là tính 'khóa lại' của Thyristor: một khi bật, nó tự duy trì dòng điện kể cả khi tín hiệu Gate mất — bạn chỉ có thể tắt nó bằng cách cắt dòng Anode hoàn toàn."
/>
...
- **Diode**: Bảo vệ, chỉnh lưu — công cụ thụ động
- **Transistor**: Điều khiển chính — đa số ứng dụng hiện đại
- **Thyristor**: Công suất siêu lớn — chuyên dụng cho hệ thống năng lượng khổng lồ

Xu hướng hiện đại: **IGBT và MOSFET** chiếm ưu thế trong hầu hết ứng dụng nhờ độ kiểm soát tốt, chi phí giảm, và hiệu suất cao — nhưng diode và thyristor vẫn chiếm chỗ đứng quan trọng ở những nơi đặc thù.